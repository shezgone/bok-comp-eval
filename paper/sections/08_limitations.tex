...
\paragraph{Production-agent specifics, and the GraphRAG / HippoRAG
absence.}
...
We explicitly do not include those external baselines in the present
paper for two reasons. First, both GraphRAG and HippoRAG build the KG
\emph{from} a text corpus and operate by community-summary or
PageRank-style retrieval — neither directly evaluates a curated
hyper-relational production KG of the kind we study. Re-tooling them
for our setting (TypeDB v3 schema, N-ary roleplayers, Korean prose
articles) requires non-trivial adaptation that we judged outside the
scope of a first-of-kind production case study. Second, our central
claim is about \emph{surface form} (Section~\ref{sec:results}), not
about retrieval mechanism, and the surface-form comparison is
internally controlled regardless of the choice of upstream retriever.
We treat external-baseline integration as the highest-priority
follow-on: a single 100-question evaluation of GraphRAG on the same
Korean regulation corpus would either confirm the surface-form
finding (NL summary remains Pareto-optimal even against community
summaries) or expose a regime in which structured retrieval recovers
the gap.
